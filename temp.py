...
def render_GP_keyframes(selected_keyframes, selected_layers):
    ...
    for pencil in bpy.data.grease_pencils:
        ...
        for layer in pencil.layers:
            ...
            if selected_layers:
                ...
                pencils_data[pencil.name]["selected_layers"].append(layer.select)
    ...
    if selected_layers:                                                                                 # ˧
        for pencil in bpy.data.grease_pencils:   
            print(pencils_data[pencil.name]["selected_layers"])                                                       # ˧
        ...
        for pencil in bpy.data.grease_pencils:                                                                                           # ˧     
            if pencil.name in pencils_data.keys():                                                                                       # ˧
                for layer in pencil.layers:                                                                                              # ˧   
                    if layer.info in pencils_data[pencil.name]["layer_name"]:                                                            # ˧
                        index = pencils_data[pencil.name]["layer_name"].index(layer.info)                                                # ˧
                        layer.select = pencils_data[pencil.name]["selected_layers"][index]                                               # ˧
...
render_GP_keyframes(0,1)

# class renderGPSettings(bpy.types.PropertyGroup):
#     selected_keyframes  : bpy.props.BoolProperty(name="Render Selected Keyframes"   , default=False)
#     selected_layers     : bpy.props.BoolProperty(name="Render Selected Layers"      , default=False)

# bpy.utils.register_class(renderGPSettings)

# bpy.types.Scene.render_gpencil_settings = bpy.props.CollectionProperty(type=renderGPSettings)

# my_variable = bpy.context.scene.render_gpencil_settings.add()
